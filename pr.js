...
const genMultiTags = ( init = {start: 1, end, step: 1, name, tag, symbol, class: `${name}-${tag}-${num}`} ) =>
{
    console.log( `^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^6` )
    console.log( `[${init.start}/${init.end}]--init.step:${init.step}` )

    const genTagsCollection = ( {stopIdx, curIdx = 0, arr = []} ) => 
    {
        if ( curIdx + 1 > init.end )
        {
            arr.push( createData( {index: curIdx, tag: init.tag, symbol: init.symbol} ) )
            console.log( curIdx, stopIdx, curIdx + 1 > init.end, arr )
            return arr
        }
        if ( curIdx === stopIdx && curIdx + init.step > init.end )
        {
            arr.push( createData( {index: curIdx, tag: init.tag, symbol: init.symbol} ) )
            console.log( curIdx, stopIdx, arr )
            return arr




            // return gentag:init.TagsCollection({stopIdx:stopIdx + init.step, curIdx:stopIdx,arr})
        }
        if ( stopIdx === curIdx && stopIdx + init.step <= init.end ) 
        {
            arr.push( createData( {index: curIdx, tag: init.tag, symbol: init.symbol} ) )
            console.log( 'dd', stopIdx, curIdx, arr )

            return genTagsCollection( {stopIdx: stopIdx + init.step, curIdx: stopIdx + init.step, arr} )
        }
    }
    let cur = init.start
    const arr = genTagsCollection( {stopIdx: cur, curIdx: cur, arr: []} )
}

const createData = ( {start, end, step, tag, symbol, name} ) => ( {index} ) => ( {code: `${tag}${index}`, open: `${tag}${index}${symbol}`, ...init, index: index} )

const genTagArrByRange = ( init = {start: 0, end, step: 1, arr: []} ) =>
{
    const {start, end, step, arr} = init
    const tagData = createData( {start, end, step, tag, symbol} )
    if ( start <= end || start + step <= end )
    {
        arr.push( start )
        return genTagArrByRange( {start: start + step, end: end, step: step, arr: arr} )
    }
    return init.arr
...
const a = genTagArrByRange( {start: 5, end: 9, step: 2, arr: []} )
const b = genTagArrByRange( {start: 2, end: 19, step: 5, arr: []} )
// const a = genMultiTags( {start: 5, end: 9, step: 2, tag: 'h', symbol: '#', name: 'title'} )  // 5,7,9
// const b = genMultiTags( {start: 1, end: 9, step: 3, tag: 'h', symbol: '#', name: 'title'} ) // 1, 4, 7
// const c = genMultiTags( {start: 1, end: 13, step: 4, tag: 'h', symbol: '#', name: 'title'} ) // 1, 5, 9, 13
// const d = genMultiTags( {start: 3, end: 9, step: 2, tag: 'h', symbol: '#', name: 'title'} ) // 1, 5, 9, 13
console.log( a )
console.log( b )
